...
uint32_t create_next_layer_rules_num = 20000;
...
uint32_t MultilayerTuple::GetReducedPrefix(uint32_t *prefix_len, Rule *rule) {
    uint32_t prefix_pair = 0;
        ...
        int a=rule->prefix_len[0];
        int b=rule->prefix_len[1];
        ...
        prefix_len[0] = 0;
        prefix_len[1] = t2;}
        else if(a >= t1 && a < t3 && b >= t2 && b < t4){
        prefix_pair = t1 << 6 | t2;
        prefix_len[0] = t1;
        prefix_len[1] = t2;}
        else if(a >= t3 && b >= t2 && b < t4){
        prefix_pair = t3 << 6 | t2;
        prefix_len[0] = t3;
        prefix_len[1] = t2;}
        else if(a >= t1 && a < t3 && b >= t4){
        prefix_pair = t1 << 6 | t4;
        prefix_len[0] = t1;
        prefix_len[1] = t4;}
        else if(a >= t3 && b >= t4){
